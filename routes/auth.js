...
// // ----------------------
// router.post('/register', async (req, res) => {
//   const { name, email, password, country, role } = req.body; // Added country
...
//       email,
//       password, // will be hashed by schema pre-save
//       country: country || 'US', // Save country (default US)
//       role: role || 'customer',
//       verificationCode,
//       isVerified: false,
//     });
...
//     // Return user profile including country
//     const userProfile = {
//       id: user._id,
//       name: user.name,
//       email: user.email,
...
// // ... (rest of the file unchanged: forgot-password, reset-password)
// module.exports = router;
...
router.post("/verify-email", async (req, res) => {
  try {
    ...
    user.isVerified = true;
    user.verificationCode = undefined; // Fixed: Clear code properly
    await user.save();
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const payload = { id: user._id, role: user.role };
    // Shortened expiry for better inactivity handling (1 hour instead of 1 day)
...
// ----------------------
// Logout (NEW: Added for complete logout)
// ----------------------
router.post('/logout', (req, res) => {
  // For stateless JWT, just respond with success. Client should discard the token.
  // If you want server-side invalidation, implement token blacklisting here (e.g., store token in DB as revoked).
